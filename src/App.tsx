    ...
    <div className="app">
      ...
      {tab === "draft" && (
        <section className="pane">
          <div className="panehead">
            <h2>Draft</h2>
            <div className="subtle">Ownership</div>
          </div>
          {state.players.map((p) => (
              ...
              <div className="lockrow">
                {TEAM_IDS.filter((t) => state.ownership[t] === p.id).map(
                  (t) => (
                    <span
                      key={t}
                      className={`lockteam ${tournamentData.alive.has(t) ? "" : "out"}`}
                    >
                      {TEAMS[t][1]} {TEAMS[t][0]}
                    </span>
                  ),
